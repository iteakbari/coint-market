...
import { CampaignData, OptionType, StatusItems } from '@/types/type'
...
function Campaigns() {
    ...
    const { register, handleSubmit, setValue, formState: { errors } } = useForm<CampaignData>();

    const { campaign } = useParams<{ campaign: string }>();
    const { data: campaignInfo } = useCampaign(campaign);

    useEffect(() => {
        setValue("Name", campaignInfo?.Name || '');
        setValue("Content", campaignInfo?.Content || '');
        setValue("Budget", campaignInfo?.Budget || '');
        setValue("StatusId", campaignInfo?.StatusId || 0);
        if (campaignInfo?.StatusId) {
            setStatusItem(campaignInfo?.StatusId)
        }
    }, [campaignInfo]);

    useEffect(() => {
        if (digitalCurrencyItems?.length > 0 && campaignInfo?.DigitalCurrencyId) {
            const selectedOption = digitalCurrencyItems.find((item: any) => item.Id === campaignInfo.DigitalCurrencyId);
            if (selectedOption) {
                setDefaultValue({
    ...
    }, [digitalCurrencyItems, campaignInfo]);

    const handleChange = (choose: any) => {
        setDefaultValue(choose);
        setDigitalCurrency(choose.value)
    }
                                ...
                                <div className='flex gap-5 pt-3'>
                                    {
                                        statusItems?.map(({ Id, Title }: StatusItems) =>
                                            <label key={Id} className='radio-label inline-flex items-center gap-1'>
                                                <input checked={statusItem === Id}
                                                    value={Id}
                                                    {...register('StatusId', { required: 'Please select status item' })}
                                                    type="radio" name="status" id={Id} onChange={() => setStatusItem(Id)} />
                                                <span>{Title}</span>
                                            </label>)
                                    }
                                </div>
